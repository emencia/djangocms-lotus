...
from djangocms_lotus.models import Article, Blog
...
def test_blog_index_empty(db, client, cms_homepage):
    ...
    """
    response = client.get("/djangocms_lotus/")
...
def test_blog_index_basic(db, client, cms_homepage):
    ...
    expected_urls = [
        "/djangocms_lotus/{blog_pk}/".format(blog_pk=blog1.id),
        "/djangocms_lotus/{blog_pk}/".format(blog_pk=blog2.id),
    ]
...
def test_blog_index_pagination(settings, db, client, cms_homepage):
    ...
    # Second result page
    response = client.get("/djangocms_lotus/?page=2")
    assert response.status_code == 200
    ...
    # Third result page
    response = client.get("/djangocms_lotus/?page=3")
...
def test_blog_detail_no_article(db, client, cms_homepage):
    ...
    url = "/djangocms_lotus/{blog_pk}/".format(blog_pk=blog1.id)
...
def test_blog_detail_article_pagination(settings, db, client, cms_homepage):
    ...
    blog_url = "/djangocms_lotus/{blog_pk}/".format(blog_pk=blog1.id)
